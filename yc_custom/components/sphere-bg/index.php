...
/**
 * the_sphere_bg();
 * @see https://codepen.io/juliangarnier/pen/LMrRNW
 */
function sphere_bg_enqueue(){
    wp_enqueue_script('anime', get_stylesheet_directory_uri() . '/yc_custom/components/sphere-bg/anime.js', array(), '3.1.0');
}
add_action( 'wp_enqueue_scripts', 'sphere_bg_enqueue' );

function sphere_bg_add_in_body()
{
    if(!is_front_page()){
        the_sphere_bg();
    }
}
add_action('wp_body_open', 'sphere_bg_add_in_body', 200);

function the_sphere_bg(){
?>
